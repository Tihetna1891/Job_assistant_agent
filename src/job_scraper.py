...
# Debugging: Check if the environment variables are loaded correctly
print(f"APP_ID: {st.secrets['general']['APP_ID']}")
print(f"API_KEY: {st.secrets['general']['API_KEY']}")

class JobScraper:
    def __init__(self, job_titles, location="London", db_name="jobs.db"):
        # Fetch sensitive data securely from secrets
        if hasattr(st, "secrets") and "general" in st.secrets:
            self.app_id = st.secrets["general"]["APP_ID"]
            self.api_key = st.secrets["general"]["API_KEY"]
        else:
            raise ValueError("API credentials not found in secrets.")

        if not self.app_id or not self.api_key:
            raise ValueError("API credentials (app_id and api_key) must be set in secrets.")
